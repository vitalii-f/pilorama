...
let lastFilteredDoc: QueryDocumentSnapshot | string = ''
...
export const FirestoreService = {
  ...
  async getFilteredArticles(lim = 1, startIndex = 0, category: string) {
    ...
    if (category !== lastFilteredCategory) {
      lastFilteredIndex = 0
      lastFilteredDoc = ''
    ...
    const dbQuery =
      startIndex >= lastFilteredIndex
        ? query(
            coll,
            orderBy('id', 'desc'),
            where('category', '==', [category]),
            startAfter(lastFilteredDoc),
            limit(lim)
          )
        : query(
            coll,
            orderBy('id', 'desc'),
            where('category', '==', [category]),
            endBefore(lastFilteredDoc),
            limit(lim)
          )

    try {
      const querySnapshot = await getDocs(dbQuery)
      lastFilteredDoc = querySnapshot.docs[querySnapshot.docs.length - 1]
      lastFilteredIndex = startIndex
  ...
  async getUsers() {
    ...
    try {
      const querySnapshot = await getDocs(
        query(collection(db, 'users'), orderBy('creationDate'))
      )
      querySnapshot.forEach((doc) => {
        const data = doc.data()
